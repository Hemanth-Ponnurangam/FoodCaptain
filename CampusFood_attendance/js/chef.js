...
// --- Attendance / QR scanning state ---
let userNameCache = null; // Map<userId, name>, lazily built from the users collection
let unsubscribeAttendance = null;
let latestAttendanceVotes = []; // raw vote docs for the currently-selected slot
let html5QrScanner = null; // active Html5Qrcode instance while the scan overlay is open
let lastScannedTicketId = null;
let lastScannedAt = 0;
...
function goToWeek(offset) {
  ...
  document.getElementById('chef-attendance-panel').hidden = true;
...
onSessionStatesChange(() => {
  ...
  if (selectedSlotId) {
    ...
    getUserNameMap().then(names => renderAttendanceTable(names));
  }
});
...
// --- Attendance (per-employee "Ate" tracking + QR scanner) ----------------

// Employee names aren't in the votes docs, so we fetch the users collection
// once and cache it. Good enough for a small campus roster; refetches if the
// scanner runs into an unknown userId (e.g. a user signed up after the cache
// was built).
async function getUserNameMap(forceRefresh) {
  if (userNameCache && !forceRefresh) return userNameCache;
  const snap = await getDocs(collection(db, 'users'));
  const map = new Map();
  snap.forEach(docSnap => map.set(docSnap.id, docSnap.data().name || docSnap.id));
  userNameCache = map;
  return map;
}

function stopAttendanceList() {
  if (unsubscribeAttendance) {
    unsubscribeAttendance();
    unsubscribeAttendance = null;
  }
  latestAttendanceVotes = [];
}

async function startAttendanceList(slot) {
  stopAttendanceList();
  const panel = document.getElementById('chef-attendance-panel');
  panel.hidden = false;
  document.getElementById('chef-attendance-subtitle').textContent =
    `Scan each employee's ticket QR code as they collect their ${slot.type.toLowerCase()} — ${slot.dayLabel}.`;
  ...
  unsubscribeAttendance = onSnapshot(
    ...
    (snapshot) => {
      latestAttendanceVotes = [];
      snapshot.forEach(docSnap => latestAttendanceVotes.push({ id: docSnap.id, ...docSnap.data() }));
      renderAttendanceTable(names);
    },
    (err) => {
      console.error('Attendance listener failed:', err);
      document.getElementById('chef-attendance-body').innerHTML =
        `<tr><td colspan="3">Couldn't load attendance (${err.code || err.message}).</td></tr>`;
    }
  );
}

// Same "does this vote actually amount to an order" logic used elsewhere
// (renderTicket in app.js, ticketTextFor above) — a "skip" or an unmet
// Other-Main-Dish-with-skip-fallback isn't an order, so it's excluded from
// the attendance list entirely rather than shown as a no-show.
function hasConfirmedOrder(voteData, extraCounts) {
  const baseId = voteData.base || voteData.option || null;
  if (baseId === 'skip') return false;
  ...
  if (anyMainUnmet && fallback === 'skip') return false;
  const usualApplies = explicitUsual || (anyMainUnmet && fallback === 'auto_regular');
  const survivors = classified.filter(ex => ex.status !== 'unmet');
  return usualApplies || survivors.length > 0;
}

function renderAttendanceTable(names) {
  const body = document.getElementById('chef-attendance-body');
  const countEl = document.getElementById('chef-attendance-count');
  const scanBtn = document.getElementById('chef-scan-open-btn');
  if (!selectedSlotId) return;

  const frozen = !!sessionStateFor(selectedSlotId).frozen;
  scanBtn.disabled = !frozen;
  if (!frozen) {
    body.innerHTML = '<tr><td colspan="3">Freeze the headcount above first — tickets aren\'t final until then.</td></tr>';
    countEl.textContent = '';
    return;
  }

  const extraCounts = {};
  latestAttendanceVotes.forEach(v => {
    (v.extras || []).forEach(ex => { extraCounts[ex.id] = (extraCounts[ex.id] || 0) + 1; });
  });

  const orders = latestAttendanceVotes
    .filter(v => hasConfirmedOrder(v, extraCounts))
    .map(v => ({ ...v, name: names.get(v.userId) || v.userId }))
    .sort((a, b) => a.name.localeCompare(b.name));

  if (!orders.length) {
    body.innerHTML = '<tr><td colspan="3">No orders yet for this session.</td></tr>';
    countEl.textContent = '';
    return;
  }

  const scannedCount = orders.filter(o => o.ate).length;
  countEl.textContent = `${scannedCount} / ${orders.length} scanned`;

  body.innerHTML = orders.map(o => `
    <tr>
      <td>${escapeHtml(o.name)}</td>
      <td>${escapeHtml(ticketTextFor(o, true, true, extraCounts))}</td>
      <td class="${o.ate ? 'attendance-status--yes' : 'attendance-status--no'}">${o.ate ? '&#10003; Scanned' : 'Not yet'}</td>
    </tr>`).join('');
}

// --- QR scanner overlay ---
function setScanResult(msg, tone) {
  const el = document.getElementById('chef-scan-result');
  el.textContent = msg;
  el.className = `chef-scan-result ${tone ? `chef-scan-result--${tone}` : ''}`;
}

async function openScanOverlay() {
  if (!selectedSlotId) return;
  const slot = findSlot(weekSlots, selectedSlotId);
  document.getElementById('chef-scan-session-label').textContent = slot ? `${slot.dayLabel} · ${slot.type}` : '';
  setScanResult('Point the camera at an employee\'s ticket QR code.', null);
  document.getElementById('chef-scan-overlay').classList.remove('hidden');

  if (typeof Html5Qrcode === 'undefined') {
    setScanResult('Scanner library failed to load — check your connection and reload.', 'error');
    return;
  }

  try {
    // eslint-disable-next-line no-undef
    html5QrScanner = new Html5Qrcode('chef-scan-reader');
    ...
    setScanResult('Could not access the camera. Check camera permissions and try again.', 'error');
  }
}

async function closeScanOverlay() {
  document.getElementById('chef-scan-overlay').classList.add('hidden');
  if (html5QrScanner) {
    try { await html5QrScanner.stop(); } catch (err) { /* already stopped */ }
    try { html5QrScanner.clear(); } catch (err) { /* no-op */ }
    html5QrScanner = null;
  }
...
async function onScanSuccess(decodedText) {
  // The scanner keeps firing while the same code is in frame — debounce
  // repeat scans of the same ticket within a few seconds.
  const now = Date.now();
  if (decodedText === lastScannedTicketId && now - lastScannedAt < 4000) return;
  lastScannedTicketId = decodedText;
  lastScannedAt = now;

  const parts = decodedText.split('__');
  const userId = parts.pop();
  const slotId = parts.join('__');

  if (!slotId || !userId) {
    setScanResult("That doesn't look like a FoodHQ ticket QR code.", 'error');
    return;
  }
  if (slotId !== selectedSlotId) {
    setScanResult("This ticket is for a different session — select that session's Headcount tab first.", 'warn');
    return;
  }

  try {
    const voteRef = doc(db, 'votes', decodedText);
    const snap = await getDoc(voteRef);
    if (!snap.exists()) {
      setScanResult('No order found for this ticket.', 'error');
      return;
    }
    const data = snap.data();
    const names = await getUserNameMap();
    const empName = names.get(userId) || userId;

    if (data.ate) {
      setScanResult(`${empName} was already scanned in.`, 'warn');
      return;
    }

    await updateDoc(voteRef, {
      ate: true,
      ateAt: serverTimestamp(),
      scannedBy: chefUserId || null
    });
    setScanResult(`\u2713 ${empName} checked in.`, 'ok');
  } catch (err) {
    console.error('Attendance scan failed:', err);
    const reason = err && err.code === 'permission-denied'
      ? 'Firestore rules are blocking this write.'
      : (err && (err.code || err.message)) || 'unknown error';
    setScanResult(`Couldn't record attendance (${reason}).`, 'error');
  }
}

document.getElementById('chef-scan-open-btn').addEventListener('click', openScanOverlay);
document.getElementById('chef-scan-close-btn').addEventListener('click', closeScanOverlay);

// --- Headcount Excel export ---
document.getElementById('chef-headcount-export-btn').addEventListener('click', () => {
  if (typeof XLSX === 'undefined') {
    alert("The Excel library didn't load (check your internet connection and reload the page).");
    return;
  }
...
// backwards to ROSTER_START_DATE. "Ate" reflects the attendance QR scan
// (the `ate` field on the vote doc, set when the Chef scans that employee's
// ticket at the counter) — "Yes"/"No" once frozen, blank before then.
// "Ticket" reproduces the same chef-approved/confirmed menu text
  ...
  const rows = employees.map(emp => {
    ...
    dates.forEach(iso => {
      ...
      SESSION_TYPES.forEach(type => {
        ...
        const ateText = voteData ? (voteData.ate ? 'Yes' : (frozen ? 'No' : '')) : '';
        row.push(ticketTextFor(voteData, eligible, frozen, extraCounts), ateText);
      });
    });
    return row;
  });
