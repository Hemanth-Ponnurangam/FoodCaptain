// --- Default/seed menu data, transcribed from the GET & DET-26 canteen boards ---
// These are only the STARTING values. The real source of truth once the app is
// running is Firestore ("config/menu"), which the Chef dashboard edits live —
// see config-store.js. This file just seeds that document the first time the
// app runs against an empty database, so the canteen doesn't start blank.
...
// Keyed by JS Date.getDay() as a STRING ("0" = Sunday ... "6" = Saturday) —
// Firestore map keys are always strings, so we keep it consistent here too.
...
// Cut-off time (24-hr "HH:MM", local time) after which a session is
// eligible to be frozen. The Chef can always freeze earlier by hand;
// this is just the "in general" fallback deadline per session type.
export const DEFAULT_CUTOFF_TIMES = {
  Breakfast: '06:30',
  Lunch: '10:00',
  Dinner: '16:00'
};

// The priced "Other Items" rate card. `threshold: true` marks items the
// canteen won't prep below THRESHOLD_MIN votes. `type` controls what
// happens if a threshold item doesn't meet quota:
//   - 'side'  — silently dropped from the ticket, no impact on the rest
//               of the order (e.g. an extra like Ice Cream not clearing).
//   - 'main'  — a real problem for the employee's meal, so they're asked
//               up front whether to fall back to the Regular/Usual Menu
//               or skip the meal entirely if it doesn't clear quota.
export const DEFAULT_OTHER_OPTIONS = [
  { id: 'egg-curry', name: 'Egg Curry', price: 25, threshold: false, type: 'side' },
  { id: 'omelette', name: 'Omelette', price: 15, threshold: false, type: 'side' },
  { id: 'chicken-masala', name: 'Chicken Masala', price: 80, threshold: true, type: 'main' },
  { id: 'mutton-fish', name: 'Mutton / Fish', price: 150, threshold: true, type: 'main' },
  { id: 'chicken-biryani', name: 'Chicken Biryani', price: 125, threshold: true, type: 'main' },
  { id: 'mutton-biryani', name: 'Mutton Biryani', price: 175, threshold: true, type: 'main' },
  { id: 'chicken-fry-chilli', name: 'Chicken (Fry/Chilli)', price: 90, threshold: true, type: 'main' },
  { id: 'paneer-mushroom', name: 'Paneer / Mushroom (Fry/Chilli)', price: 80, threshold: true, type: 'main' },
  { id: 'ice-cream', name: 'Ice Cream', price: null, priceLabel: 'Actuals', threshold: false, type: 'side' }
];
