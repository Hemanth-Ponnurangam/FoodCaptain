...
export function slotCutoffDate(slot, cutoffTimes) {
  ...
  const cutoff = new Date(y, mo - 1, d, h, m, 0, 0);
  // Breakfast is served first thing in the morning, so its deadline always
  // falls the evening before (Day 2's Breakfast deadline is on Day 1) —
  // shift the anchor date back one day before applying the configured time.
...
export function formatCutoffLabel(slot, cutoffTimes) {
  const cutoff = slotCutoffDate(slot, cutoffTimes);
  const timeLabel = cutoff.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' });
  if (slot.type === 'Breakfast') {
    const dayLabel = cutoff.toLocaleDateString('en-US', { weekday: 'long' });
    return `${timeLabel}, ${dayLabel} (the evening before)`;
  }
  return timeLabel;
}
