...
// The fallback choice only matters if at least one selected extra actually
// has a headcount minimum — hide the question otherwise.
function updateFallbackVisibility(slot) {
  const lookup = allOptionsLookup(slot);
  const anyThresholdChecked = Array.from(document.querySelectorAll('input[name="meal-extra"]:checked'))
    .some(cb => lookup[cb.value] && lookup[cb.value].threshold);
  document.getElementById('fallback-group').classList.toggle('hidden', !anyThresholdChecked);
}
...
function renderTicket(slot, frozen) {
  ...
  getDoc(doc(db, 'votes', voteDocId()))
    .then(snap => {
      ...
      const anyUnmet = classified.some(ex => ex.status === 'unmet');

      if (frozen) {
        if (anyUnmet && fallback === 'skip') {
          container.innerHTML = `
            <div class="ticket-status-banner ticket-status-banner--skipped">Your special item didn't meet the ${THRESHOLD_MIN}-vote minimum, so per your preference this order was skipped — no charge.</div>`;
          return;
        }
        ...
        const rows = [
          ...
          ...classified.map(ex => {
            ...
            const note = ex.status === 'unmet' ? ` <em>(${ex.count}/${THRESHOLD_MIN} votes)</em>`
...
async function loadVote() {
  ...
  try {
    ...
    if (existing.exists()) {
      ...
      const fallbackRadio = document.querySelector(`input[name="fallback-choice"][value="${CSS.escape(data.fallbackChoice || 'auto_regular')}"]`);
...
document.getElementById('vote-form').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    const fallbackChoice = document.querySelector('input[name="fallback-choice"]:checked')?.value || 'auto_regular';
