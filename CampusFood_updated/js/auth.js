...
// Chef accounts require this passcode at signup so random users can't just
// tick "Chef" and get chef-only access. Same "not real security" caveat as
// everything else in this file applies -- it's a shared secret in client JS,
// not a server-enforced check.
const CHEF_SIGNUP_PASSCODE = 'GET2026@MDC';
...
// Show the chef passcode field only when "Chef" is selected.
const chefPasscodeField = document.getElementById('signup-chef-passcode');
document.querySelectorAll('input[name="signup-role"]').forEach((radio) => {
  radio.addEventListener('change', (e) => {
    const isChef = e.target.value === 'chef';
    chefPasscodeField.classList.toggle('hidden', !isChef);
    if (!isChef) chefPasscodeField.value = '';
  });
});
...
document.getElementById('signup-form').addEventListener('submit', async (e) => {
  ...
  if (role === 'chef') {
    const passcode = chefPasscodeField.value.trim();
    if (passcode !== CHEF_SIGNUP_PASSCODE) {
      showError('signup-error', 'Incorrect chef security passcode.');
      return;
    }
  }
